...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['date'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM booked_dates WHERE booked_date = ?");
    $stmt->bind_param("s", $date);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if ($result->num_rows > 0) {
        $message = "Sorry, this date is already booked.";
        $dateAvailable = false;
    } else {
        $message = "The date is available for booking!";
    }
    $stmt->close();
}

// Fetch all booked dates
$sql = "SELECT * FROM booked_dates ORDER BY booked_date";
...
<body>
<div class="container">
    <h2><i class="fas fa-calendar-check mr-2"></i>Check Booking Availability</h2>
    ...
    <button type="button" class="btn btn-info btn-block mt-3" data-toggle="modal" data-target="#bookedDatesModal">
        <i class="fas fa-list mr-2"></i>See All Booked Dates
    </button>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="bookedDatesModalLabel">All Booked Dates</h5>
